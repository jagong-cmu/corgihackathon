...
def check_env(report: Report) -> None:
    ...
    for name, why in REQUIRED.items():
        ...
        if not value:
            report.add(
                Result(name, Status.FAIL, f"unset — needed for {why}",
                       "set -a && . ../../.env.local && set +a")
            )
        elif value in PLACEHOLDERS.get(name, set()):
            report.add(
                Result(name, Status.FAIL, f"still the dev placeholder ({value!r})",
    ...
    if url and not url.startswith(("ws://", "wss://")):
        report.add(
            Result("LIVEKIT_URL scheme", Status.FAIL, f"{url!r} is not a websocket URL",
...
def check_persona(report: Report, persona_id: str):
    ...
    try:
        ...
        report.add(Result("load", Status.FAIL, f"{type(exc).__name__}: {exc}",
    ...
    if persona.voice is None:
        report.add(Result("voice", Status.FAIL, "no voice configured",
                          "a realtime persona needs voice.voice_id — the loop cannot speak"))
    else:
        report.add(Result("voice", Status.PASS,
                          f"{persona.voice.voice_id} via {persona.voice.model}"))
...
async def check_anthropic(report: Report) -> None:
    ...
    try:
        ...
        report.add(Result("turn with canvas tools", Status.FAIL, f"{type(exc).__name__}: {exc}",
...
async def check_elevenlabs_tts(report: Report, persona) -> None:
    ...
    try:
        ...
        report.add(
            Result(f"pcm_{SAMPLE_RATE} stream", Status.PASS,
                   f"{len(audio)} bytes", timing_ms=first_chunk_ms)
        )
        ...
        if joined == text:
            ...
            report.add(
                Result("character alignment", Status.FAIL,
        ...
        report.add(
            Result("sample-boundary chunks", Status.PASS,
        ...
        if 0.5 <= duration_s <= 5.0:
            ...
            report.add(
                Result("decoded duration", Status.FAIL, f"{duration_s:.2f}s — implausible",
        ...
        if first_chunk_ms is not None and first_chunk_ms > BUDGET_MS * 0.5:
            report.add(
                Result("tts headroom", Status.WARN,
        ...
        report.add(Result("synthesize", Status.FAIL, f"{type(exc).__name__}: {exc}",
...
async def check_elevenlabs_stt(report: Report) -> None:
    ...
    try:
        ...
        report.add(
            Result("realtime stt", Status.PASS,
        ...
        if silence > BUDGET_MS / 2:
            report.add(
                Result("vad headroom", Status.WARN,
        ...
        report.add(Result("realtime stt", Status.FAIL, f"{type(exc).__name__}: {exc}",
...
async def check_livekit(report: Report, room_name: str) -> None:
    ...
    try:
        ...
        report.add(
            Result("connect", Status.PASS, f"sid {await room.sid}",
                   timing_ms=(time.perf_counter() - started) * 1000)
        )
    except Exception as exc:
        report.add(Result("connect", Status.FAIL, f"{type(exc).__name__}: {exc}",
    ...
    try:
        ...
        report.add(Result("publish ragged pcm", Status.PASS,
        ...
        report.add(Result("data channel", Status.PASS,
...
async def check_avatar(report: Report, persona) -> None:
    ...
    if provider == "lemonslice":
        ...
        report.add(Result("configured", Status.FAIL, f"unknown provider {provider!r}",
                          "supported: lemonslice, simli, none"))
        return

    if not os.environ.get(key):
        report.add(Result(key, Status.WARN, "unset — the session degrades to voice-only",
    ...
    if not ref:
        report.add(Result("avatar ref", Status.WARN, f"neither persona.avatar_ref nor {ref_env}",
    ...
    if provider == "simli" and ref:
        try:
            ...
            if response.status_code < 400:
                ...
                report.add(Result("simli session token", Status.FAIL,
        ...
        report.add(Result("handshake", Status.SKIP,
                          "only completes inside a job context — run the worker"))
...
async def check_retrieval(report: Report) -> None:
    ...
    try:
        ...
        report.add(Result("driver", Status.FAIL, "asyncpg missing",
    ...
    try:
        ...
        if applied:
            ...
            report.add(Result("schema", Status.FAIL, "doc_chunks migration not applied",
        ...
        if dim == EMBEDDING_DIM:
            ...
            report.add(
                Result("embedding width", Status.FAIL,
        ...
        if indexed:
            ...
            report.add(Result("indexed chunks", Status.WARN, "index is empty",
                              "uv run tutor ingest <file> --user <uuid> --upload <uuid>"))

        if not os.environ.get("VOYAGE_API_KEY"):
            report.add(
                Result("embedder", Status.WARN, "VOYAGE_API_KEY unset — hashing fallback",
